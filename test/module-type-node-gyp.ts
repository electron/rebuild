...
describe('node-gyp', function() {
  describe('buildArgs', function() {
    ...
    context('sufficiently old Electron versions which lack a bundled config.gypi', function() {
      it('adds --force-process-config for < 14', async function() {
        const args = await nodeGypArgsForElectronVersion('12.0.0');
        expect(args).to.include('--force-process-config');
      });

      it('adds --force-process-config for between 14.0.0 and < 14.2.0', async function() {
        const args = await nodeGypArgsForElectronVersion('14.1.0');
        expect(args).to.include('--force-process-config');
      });

      it('adds --force-process-config for versions between 15.0.0 and < 15.3.0', async function() {
        const args = await nodeGypArgsForElectronVersion('15.2.0');
        expect(args).to.include('--force-process-config');
      });
    });

    context('for sufficiently new Electron versions', function() {
      it('does not add --force-process-config for ^14.2.0', async function() {
        const args = await nodeGypArgsForElectronVersion('14.2.0');
        expect(args).to.not.include('--force-process-config');
      });

      it('does not add --force-process-config for ^15.3.0', async function() {
        const args = await nodeGypArgsForElectronVersion('15.3.0');
        expect(args).to.not.include('--force-process-config');
      });

      it('does not add --force-process-config for >= 16.0.0', async function() {
