...
describe(
  'rebuild with napi_build_versions in binary config',
  { timeout: TIMEOUT_IN_MILLISECONDS },
  () => {
    const napiBuildVersion = 6;
    const napiBuildVersionSpecificPath = (arch: string, libc: string) =>
      path.resolve(
        testModulePath,
        `node_modules/node-pre-gyp-test/lib/binding/napi-v${napiBuildVersion}-${process.platform}-${libc}-${arch}/node_sqlite3.node`,
      );
