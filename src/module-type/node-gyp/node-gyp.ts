...
export class NodeGyp extends NativeModule {
  async buildArgs(prefixedArgs: string[]): Promise<string[]> {
    ...
    if (
      !semver.satisfies(this.rebuilder.electronVersion, '^14.2.0 || ^15.3.0') &&
      semver.major(this.rebuilder.electronVersion) < 16
    ) {
  ...
  async rebuildModule(): Promise<void> {
    ...
    if (this.rebuilder.useElectronClang) {
      const { env: clangEnv, args: clangArgs } = await getClangEnvironmentVars(
